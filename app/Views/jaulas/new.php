...
<h1 class="mb-4">Agregar Jaulas Trampa</h1>
...
<div class="card">
	<div class="card-body text-center">
		...
		<form class="text-start" action="<?= base_url('jaulas/store') ?>" method="post">
			<?= csrf_field() ?>
			...
			<div class="col-12">
				<div class="row mb-3">
					<div class="col-6">
						...
						<input type="text" class="form-control" name="etiqueta" id="etiqueta"
							value="<?= esc(old('etiqueta')) ?>" maxlength="20" required autofocus style="width: 20ch">
					</div>
					<div class="col-6">
						...
						<select name="estado" id="estado" class="form-select w-auto">
							<option value="1" <?= old('estado') == '1' ? 'selected' : '' ?>>Útil</option>
							<option value="0" <?= old('estado') == '0' ? 'selected' : '' ?>>No útil</option>
						</select>
					</div>
				</div>
				<div class="row mb-3">
					<div class="col-12">
						...
						<input type="text" class="form-control" name="descripcion" id="descripcion"
							value="<?= esc(old('descripcion')) ?>" required autofocus>
					</div>
				</div>
				<div class="row mb-3">
					<div class="col-6">
						...
						<select name="propietario" id="propietario" class="form-select">
							<option value="La Protectora"
								<?= old('propietario') == 'La Protectora' ? 'selected' : '' ?>>
								Asociación Protectora
							</option>
							<option value="0" disabled>-----------------------------</option>
							<?php foreach ($socios as $socio): ?>
							<option value="<?= esc($socio->nombre) ?>"
								<?= old('propietario') == $socio->nombre ? 'selected' : '' ?>>
								<?= esc($socio->nombre) ?>
							</option>
							<?php endforeach; ?>
						</select>
				...
				<div class="mb-3">
					...
					<textarea name="caracteristicas" id="caracteristicas"
						class="form-control"><?= esc(old('caracteristicas')) ?></textarea>
				</div>
